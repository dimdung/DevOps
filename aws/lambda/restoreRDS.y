...
def lambda_handler(event, context):
    ...
    for instance in instances:
        try:
            ...
            snap_id = (re.sub( '-\d\d-\d\d-\d\d\d\d-\d\d-\d\d ?', '', source_snap))
            print('Will restore %s to %s' % (source_snap, snap_id))
            response = source.restore_db_instance_from_db_snapshot(DBInstanceIdentifier=snap_id,
            DBSnapshotIdentifier=source_snap, DBInstanceClass=db_instance_class,
            DBSubnetGroupName=db_subnet,MultiAZ=False,PubliclyAccessible=False)
            print(response)
